...
\NormalTok{ebd <-}\StringTok{ }\NormalTok{ebd[(longitude }\OperatorTok{%in%}\StringTok{ }\NormalTok{coords}\OperatorTok{$}\NormalTok{longitude) }\OperatorTok{&}\StringTok{ }
...
\hypertarget{species-observed-in-relation-to-osberver-expertise}{%
\subsection{Species observed in relation to osberver expertise}\label{species-observed-in-relation-to-osberver-expertise}}
...
\NormalTok{  assertthat}\OperatorTok{::}\KeywordTok{assert_that}\NormalTok{(}\StringTok{"RasterLayer"} \OperatorTok{%in%}\StringTok{ }\KeywordTok{class}\NormalTok{(inp),}
...
\NormalTok{lims <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}
  \KeywordTok{range}\NormalTok{(chelsa[[}\DecValTok{1}\NormalTok{]]}\OperatorTok{$}\NormalTok{bio_}\DecValTok{01}\NormalTok{, }\DataTypeTok{na.rm =}\NormalTok{ T),}
  \KeywordTok{range}\NormalTok{(chelsa[[}\DecValTok{2}\NormalTok{]]}\OperatorTok{$}\NormalTok{bio_}\DecValTok{12}\NormalTok{, }\DataTypeTok{na.rm =}\NormalTok{ T)}
\NormalTok{)}
...
\KeywordTok{ggsave}\NormalTok{(}
  \DataTypeTok{plot =}\NormalTok{ plot_grid, }\DataTypeTok{filename =} \StringTok{"figs/fig_chelsa_variograms.png"}\NormalTok{,}
  \DataTypeTok{dpi =} \DecValTok{300}\NormalTok{, }\DataTypeTok{width =} \DecValTok{12}\NormalTok{, }\DataTypeTok{height =} \DecValTok{6}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{\{}
\NormalTok{  data <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(}\StringTok{"data/eBirdChecklistVars.csv"}\NormalTok{) }\OperatorTok{%>%}
...
\NormalTok{    i_n_d <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(mods, coordId }\OperatorTok{%in%}\StringTok{ }\NormalTok{nodes_a_init) }\OperatorTok{%>%}
...
\NormalTok{    b <-}\StringTok{ }\NormalTok{i_n_d }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(node }\OperatorTok{%in%}\StringTok{ }\NormalTok{nodes_a)}
...
\NormalTok{      indep_sampling <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(b, node }\OperatorTok{%in%}\StringTok{ }\NormalTok{indep_nodes) }\OperatorTok{%>%}
...
\NormalTok{data <-}\StringTok{ }\KeywordTok{mutate}\NormalTok{(data,}
  \DataTypeTok{data =} \KeywordTok{map}\NormalTok{(sites_removed, }\ControlFlowTok{function}\NormalTok{(site_id) \{}
    \KeywordTok{as_tibble}\NormalTok{(}\KeywordTok{filter}\NormalTok{(ul, }\OperatorTok{!}\NormalTok{coordId }\OperatorTok{%in%}\StringTok{ }\NormalTok{site_id))}
\NormalTok{  \})}
\NormalTok{)}
...
This script was used to classify a 2019 Sentinel composite image across the Nilgiris and the Anamalais into seven distinct land cover classes

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\CommentTok{// Data: Groundtruthed points from Arasumani }
